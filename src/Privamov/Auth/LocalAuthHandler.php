...
/**
 * Authentication against a static array of users.
 *
 * @author Vincent Primault <vincent.primault@liris.cnrs.fr>
 */
class LocalAuthHandler implements AuthHandler
{
    private $users;
    ...
    {
        if (isset($this->users[$login]) && $this->users[$login] === $password) {
            return ['username' => ucwords($login)];
        }
        return false;
    }
